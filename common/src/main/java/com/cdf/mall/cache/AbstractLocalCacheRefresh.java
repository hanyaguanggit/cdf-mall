 ...
 * @Version 1.0
 */
public abstract class AbstractLocalCacheRefresh<T> {
    private static final Logger log = LoggerFactory.getLogger(AbstractLocalCacheRefresh.class);
    ...
    public AbstractLocalCacheRefresh() {
    }
    ...
    private void build() {
        ...
        this.loadingCache = Caffeine.newBuilder().recordStats().maximumSize(this.maximumSize()).expireAfterWrite(duration + 0L, TimeUnit.SECONDS).build(new CacheLoader<String, T>() {
            public T load(String key) {
                AbstractLocalCacheRefresh.log.debug("local cache load, cacheName={}, key={}", AbstractLocalCacheRefresh.this.cacheName(), key);

                try {
                    T value = AbstractLocalCacheRefresh.this.load(key);
                    if (value == null && !AbstractLocalCacheRefresh.this.nullable()) {
                        AbstractLocalCacheRefresh.log.debug("local cache load return null, cacheName={}, key={}", AbstractLocalCacheRefresh.this.cacheName(), key);
                    }

                    return value;
                } catch (Exception var3) {
                    AbstractLocalCacheRefresh.log.error("local cache load error, cacheName={}, key={}", new Object[]{AbstractLocalCacheRefresh.this.cacheName(), key, var3});
                    return null;
                }
            }
        });
